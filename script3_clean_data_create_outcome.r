...
#####
##### Arguments expected:
#####     #1 -- Path to and name of the phenotype file, for example /hpc/dhl_ec/mvanvugt/UKBB/Project1_ukb_phenotypes.tab
#####     #2 -- Directory to the pipeline, for example /hpc/dhl_ec/mvanvugt/Software/UKB-pipeline-Utrecht
...
message("load data")
#read file
d <- data.table(read.delim(pheno, sep = '\t', header=T))
...
for(i in 1:nrow(outcomes)){
  ...
  message(NAME)
  x <- read.csv(file = paste0(scripts, "/CALIBER/secondary_care/", outcomes$ICD[i]))
  ...
  d[[paste0(NAME)]] <- as.integer(apply(d[,..ICD10], 1, function(r) any(r %in% c(grep(y, r, value =T))))) +
    ...
    if(outcomes$OPCS[i] != ""){
      xx <- read.csv(file = paste0(scripts, "/CALIBER/secondary_care/", outcomes$OPCS[i]))
...
save(d, file= paste0(output, "/checkpoint2.rda"))
...
draw <- d
save(draw, file= paste0(output, "/ICD_outcome_raw.rda"))
...
save(d, file= paste0(output, "/ICD_outcome_cleaned.rda"))
